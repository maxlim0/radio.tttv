...
jQuery("#list3").jqGrid({
	...
	width: 1075,
	datatype: "json",
   	colNames:['<img src="images/lock.png" />','<img src="images/user1.gif" />', 'Логин','ФИО','Телефон','Пароль','E-mail','Дата регистрации','IP рег-ии','Страна рег-ии','Город рег-ии','Дата последнего входа','Статус'],
   	colModel:[
	...
	*/
	...
	$( "#add_news_chat_us" ).click(function() {
		...
		$('#add_closed_conference').click(function() {
						arr = jQuery("#list3").jqGrid('getGridParam','selarrrow'); alert(arr);
		...
		$( "#work_add" )
			...
			.click(function() {
				...
				$("#list3").setGridParam({url:"vchat.php?check=3&name="+name_add+"&fio="+fio_add+"&phone="+phone_add+"&pass="+pass_add+"&mail="+mail_add+"&role="+role_add+"",page:1}).trigger('reloadGrid');					
				$( "#dialog_add_users" ).dialog( "close" );
		...
		$( "#work_edit" )
			...
			.click(function() {
				if($('#name_edit').val()==''){
					...
					$("#list3").setGridParam({url:"vchat.php?check=5&name="+name_edit+"&fio="+fio_edit+"&phone="+phone_edit+"&newpass="+pass_edit+"&mail="+mail_edit+"&role="+role_edit+"&id="+id_edit+"",page:1}).trigger('reloadGrid');	
...
function f_UsersOptions(){
		$opt = $("#regmoder_users :selected").val();
